...
enum class pointer_kind_t { eNullable, eNonNullable };
...
struct pointer_t {
  ...
  deref() const;
  static SP<type_t>
  pointer_to(pointer_kind_t, SP<type_t>, bool = false);
};
...
struct type_t {
  ...
  bool
  operator==(const type_t &other) const;
  ...
  static SP<type_t>
  make_slice(SP<type_t>, bool is_mutable);
};
...
size_t
size_of(const type_t &);
size_t
alignment_of(const type_t &);

std::string
            to_string(const type_t &);
