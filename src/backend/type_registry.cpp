...
SP<type_t>
type_registry_t::resolve(const specialized_path_t &path) {
  std::string name = to_string(path);
  return resolve(name);
}

SP<type_t>
type_registry_t::add_builtin(const specialized_path_t &name,
                             size_t                    size,
                             size_t                    alignment,
                             type_kind_t               kind) {
...
SP<type_t>
type_registry_t::add_function(SP<type_t>                     return_type,
...
SP<type_t>
type_registry_t::add_struct(const specialized_path_t &name, struct_layout_t layout) {
...
SP<type_t>
type_registry_t::add_contract(const specialized_path_t                &name,
...
SP<type_t>
type_registry_t::add_alias(const specialized_path_t &name, SP<type_t> alias, bool is_distinct) {
  auto type       = std::make_shared<type_t>();
...
SP<type_t>
type_registry_t::add_template_alias(const specialized_path_t &name, SP<type_t> alias) {
  auto type       = std::make_shared<type_t>();
  type->size      = size_of(alias);
  type->alignment = alignment_of(alias);
...
SP<type_t>
type_registry_t::pointer_to(SP<type_t>                  base,
...
SP<type_t>
type_registry_t::array_of(SP<type_t> base, size_t len) {
  auto type       = std::make_shared<type_t>();
  type->size      = sizeof(void *);
  type->alignment = sizeof(void *);
...
SP<type_t>
type_registry_t::slice_of(SP<type_t> base, bool is_mutable) {
...
SP<type_t>
type_registry_t::self_placeholder(const specialized_path_t &owner_name) {
  auto t  = std::make_shared<type_t>();
  t->kind = eSelf;
  t->name = owner_name;
  return t;
...
SP<type_t>
type_registry_t::untyped_literal(const std::string &value, literal_type_t ty) {
  auto type        = std::make_shared<type_t>();
  type->size       = 0;
  type->kind       = type_kind_t::eUntypedLiteral;
  type->as.literal = new untyped_literal_t{ .value = value, .type = ty };
  return type;
}
