...
void
A::import_source_file(const std::string &path) {
...
A::analyze(translation_unit_t tu) {
  push_scope();
  info =
    std::make_unique<semantic_info_t>(semantic_info_t{ .unit = tu, .scope = scope_stack.front() });
  ...
  for (auto &import_ : info->unit.imports) {
    ...
    if (!found) {
      diagnostics.messages.push_back(error(source,
                                           {
                                             { 0, 0 },
                                             { 0, 0 }
      },
                                           "Unknown import",
                                           "Import " + to_string(import_) + " wasn't found"));
  ...
  if (base) {
    ...
    if (ty.tuple) {
      ...
      for (auto &[k, v] : ty.tuple->elements) {
        tuple_types.emplace_back(k.value_or(std::to_string(nmemb)),
                                 ensure_concrete(resolve_type(v)));
        nmemb++;
      }
...
QT
A::resolve_type(const specialized_path_t &path) {
  type_decl_t decl{
...
QT
A::resolve_type(const std::string &name) {
  type_decl_t decl{
  ...
  for (auto &param : decl.parameters) {
    if (param.is_self) {
      ...
      if (param.is_self_ref) {
        active_receiver = scope().types.pointer_to(
          active_receiver, { pointer_kind_t::eNonNullable }, param.is_mutable);
      }
    } else {
    ...
    if (params.size() > 0 && !params.back()) {
      diagnostics.messages.push_back(error(source,
                                           ...
                                           "Unknown type",
                                           fmt("Unknown type {}", to_string(param.type))));
      throw analyze_error_t{ diagnostics };
    }
  }
  ...
  if (*actual_return_type != *expected_return_type &&
    ...
    diagnostics.messages.push_back(
      error(node->source,
            node->location,
            "Type mismatch",
            fmt("Function is supposed to return `{}`, but block evaluates to `{}`",
                to_string(expected_return_type),
                to_string(actual_return_type))));
    throw analyze_error_t{ diagnostics };
  }
  return function;
  ;
...
value_category_t
A::resolve_value_category(N node) {
  switch (node->kind) {
    case ast_node_t::eSymbol:
      return value_category_t::eLValue;
    case ast_node_t::eMemberAccess:
      return value_category_t::eLValue; // x.y
    case ast_node_t::eMove:
      return value_category_t::eRValue; // move x
    case ast_node_t::eLiteral:
      return value_category_t::eRValue; // 5, "hi"
    case ast_node_t::eCall:
      return value_category_t::eRValue; // f()
    default:
      return value_category_t::eRValue;
  }
}
...
A::analyze_call(N node) {
  ...
  if (fn->kind != type_kind_t::eFunction) {
    diagnostics.messages.push_back(
      error(call->callee->source,
            call->callee->location,
            "Not callable",
            "This expression does not evaluate to a function, and thus is not callable."));
    throw analyze_error_t{ diagnostics };
  }
  ...
  if (required_parameters != passed_arguments &&
    ...
    diagnostics.messages.push_back(
      error(node->source,
            ...
            fmt("Function expects {} arguments, got {}", required_parameters, passed_arguments)));
    throw analyze_error_t{ diagnostics };
  }
  ...
  for (auto i = 0; i < signature->arg_types.size(); ++i) {
    ...
    if (param->kind == type_kind_t::eRValueReference) {
      if (resolve_value_category(arg_node) != value_category_t::eRValue) {
        diagnostics.messages.push_back(
          error(arg_node->source,
                arg_node->location,
                "Move required",
                "Cannot pass an lvalue to a move-only parameter (^T). Use 'move'."));
        throw analyze_error_t{ diagnostics };
      }
    }
    ...
    if (!is_implicit_convertible(arg_type, param)) {
      diagnostics.messages.push_back(error(
        call->arguments[i]->source,
        call->arguments[i]->location,
        "Invalid type",
        fmt("Expected {}, got {} for argument #{}", to_string(param), to_string(arg_type), i)));
      throw analyze_error_t{ diagnostics };
    }
  }
  ...
  if (is_dynamic_dispatch(node) && has_receiver && !call->implicit_receiver) {
    ...
    full_symbol_path.insert(full_symbol_path.begin(),
                            signature->receiver->name.segments.begin(),
                            signature->receiver->name.segments.end());

    // e.g. `self.member` path to the receiver
    std::vector<specialized_segment_t> receiver_path;
    receiver_path.insert(receiver_path.begin(),
                         qualified_symbol_path.segments.begin(),
                         qualified_symbol_path.segments.end() - 1);

    call->implicit_receiver = make_node<symbol_expr_t>(
      ast_node_t::eSymbol, { receiver_path }, node->location, node->source);
    ...
    if (signature->receiver->kind == type_kind_t::ePointer &&
      ...
      call->implicit_receiver = make_node<addr_of_expr_t>(
        ast_node_t::eAddrOf, { call->implicit_receiver }, node->location, node->source);
      // TODO: Refactor this...
      //
    ...
    call->callee = make_node<symbol_expr_t>(
      ast_node_t::eSymbol, { full_symbol_path }, node->location, node->source);
...
A::analyze_declaration(N node) {
    ...
    if (inferred_type->kind == type_kind_t::eUntypedLiteral && annotated_type) {
      if (annotated_type->is_numeric()) {
        if (can_fit_literal(inferred_type->as.literal->value, annotated_type)) {
          ...
          diagnostics.messages.push_back(error(node->source,
                                               node->location,
                                               "Literal overflow",
                                               fmt("Value {} does not fit in type {}.",
                                                   inferred_type->as.literal->value,
                                                   to_string(annotated_type))));
          throw analyze_error_t{ diagnostics };
        }
      }
    }
    ...
    if (*annotated_type != *inferred_type &&
      ...
      diagnostics.messages.push_back(
        error(node->source,
              node->location,
              "Type mismatch",
              fmt("Type annotation holds a different type than the expression. ({} vs {})",
                  to_string(annotated_type),
  ...
  if (!final_type) {
    diagnostics.messages.push_back(
      error(node->source, node->location, "Type error", "Cannot infer type."));
    throw analyze_error_t{ diagnostics };
  }
...
A::resolve_tuple_element(QT tuple, const std::string &member_name) {
  ...
  if (tuple_layout->element(member_name) == tuple_layout->elements.end()) {
    diagnostics.messages.push_back(error(source,
                                         ...
                                         fmt("Tuple element {} is unknown", member_name)));
    throw analyze_error_t{ diagnostics };
  }
...
A::resolve_enum_element(QT enum_, const std::string &member_name) {
  if (!enum_->as.enum_->values.contains(member_name)) {
    diagnostics.messages.push_back(error(source,
                                         ...
                                         "Invalid enum element",
                                         fmt("Enum element {} is unknown", member_name)));
    throw analyze_error_t{ diagnostics };
  }
  return enum_;
}
...
A::resolve_member_access(QT base, const std::string &member_name) {
  switch (base->kind) {
    ...
    case type_kind_t::eArray: {
      auto element = base->kind == type_kind_t::eSlice ? base->as.slice->element_type
                                                       : base->as.array->element_type;
      if (member_name == "size")
        return resolve_type("u64");
      if (member_name == "data")
        return scope().types.pointer_to(
          element, { pointer_kind_t::eNonNullable }, base->kind == type_kind_t::eSlice);
      break;
    }
...
A::analyze_symbol(N node) {
  ...
  // Since we don't disambiguate between namespaces, etc., we have to do some special checks on this
  // symbol now.
  ...
  // 3. `a.b` refers to a UFCS, where `a` is a local variable, and `b` refers to a function named
  // like `<type of a>.b`
  ...
  if (left) { // UFCS
    ...
    ufcs.segments.insert(
      ufcs.segments.begin(), left->name.segments.begin(), left->name.segments.end());
  ...
  for (auto it = path.segments.begin(); it != path.segments.end();) {
    ...
    path.segments.insert(
      path.segments.begin() + pos, ty->name.segments.begin(), ty->name.segments.end());
  ...
  diagnostics.messages.emplace_back(
    error(source,
          node->location,
          "Unknown symbol",
          fmt("Symbol `{}` is not known in the current scope.", to_string(path))));
  throw analyze_error_t{ diagnostics };
}
...
bool
A::is_lvalue(N node) {
  return node->kind == ast_node_t::eSymbol || node->kind == ast_node_t::eMemberAccess ||
         node->kind == ast_node_t::eArrayAccess || node->kind == ast_node_t::eDeref;
}
...
A::analyze_deref(N node) {
  ...
  if (is_rvalue(expr->value) && value_type->kind != type_kind_t::ePointer) {
    diagnostics.messages.push_back(error(node->source,
                                         node->location,
                                         "Dereferencing R-value",
                                         "This expression is an rvalue (temporary), and has no "
                                         "address, taking a reference of this is not allowed."));
    throw analyze_error_t{ diagnostics };
  }
  ...
  if (value_type->kind != type_kind_t::ePointer) {
    diagnostics.messages.push_back(
      error(node->source,
            node->location,
            "Dereferencing concrete type",
            "This expression resolves to a concrete type, expected a pointer."));
    throw analyze_error_t{ diagnostics };
  }
...
A::analyze_struct_expr(N node) {
  ...
  if (!struct_type) {
    diagnostics.messages.push_back(
      error(source, node->location, "Unknown struct type", fmt("Type doesn't exist")));
    throw analyze_error_t{ diagnostics };
  }

  if (struct_type->kind != type_kind_t::eStruct) {
    diagnostics.messages.push_back(
      error(source, node->location, "Invalid type", fmt("This type is not a struct")));
    throw analyze_error_t{ diagnostics };
  }
    ...
    if (layout->member(member_name) == nullptr) {
      diagnostics.messages.push_back(
        error(source,
              node->location,
              "Unknown member",
              fmt("Member {} doesn't exist on this struct.", member_name)));
      throw analyze_error_t{ diagnostics };
    }
    analyze_node(value);
  }
...
A::analyze_cast(N node) {
  ...
  if (!is_cast_convertible(value_type, cast_type)) {
    diagnostics.messages.push_back(error(node->source,
                                         node->location,
                                         "Invalid cast",
                                         fmt("Casting from `{}` to `{}` is not possible.",
...
A::satisfies_contract(QT type, QT contract_type) {
  ...
  for (auto &[req_name, req_type] : contract->requirements) {
    if (req_type->kind == type_kind_t::eFunction) {
      ...
      auto sym = scope().resolve(specialized_path_t({
        { to_string(type->name), {} },
        {              req_name, {} }
      }));
      if (!sym) {
      ...
      if (sym->type->kind != type_kind_t::eFunction) {
        diagnostics.messages.push_back(
          error(source,
                ...
                "Contract violation",
                fmt("`{}` does not implement the contract function `{}` (got {}, expected {})",
                    sym->name,
                    req_name,
                    to_string(sym->type),
                    to_string(req_type))));
      ...
      if (symbol_fn->arg_types.size() != contract_fn->arg_types.size()) {
        diagnostics.messages.push_back(
          error(source,
                ...
                "Contract violation",
                fmt("Function `{}` does not match the contract function `{}` (got {}, expected {})",
                    sym->name,
                    req_name,
                    to_string(sym->type),
                    to_string(req_type))));
      ...
      if ((symbol_fn->receiver.get() != nullptr) != (contract_fn->receiver.get() != nullptr)) {
        diagnostics.messages.push_back(
          error(source,
                {
                  { 0, 0 },
                ...
                "Contract violation",
                fmt("Function `{}` does not match the contract function `{}`, receiver present on "
      ...
      if (type->kind != type_kind_t::eStruct) {
        diagnostics.messages.push_back(
          error(source,
                ...
                "Contract violation",
                fmt("Type `{}` can't satisfy contract `{}` due to member requirement `{}`",
                    to_string(type->name),
                    to_string(contract_type->name),
                    req_name)));
        return false;
      }
      ...
      if (!member) {
        diagnostics.messages.push_back(
          error(source,
                ...
                "Contract violation",
                fmt("Type `{}` can't satisfy contract `{}` due to missing member `{}`",
                    to_string(type->name),
                    to_string(contract_type->name),
                    req_name)));
        return false;
      ...
      if (member->type != req_type) {
        diagnostics.messages.push_back(error(source,
                                             ...
                                             "Contract violation",
                                             fmt("Type `{}` can't satisfy contract `{}` due to "
                                                 "member `{}` type mismatch (got {}, expected {})",
                                                 to_string(type->name),
                                                 to_string(contract_type->name),
                                                 req_name,
...
A::is_implicit_convertible(QT from, QT into) {
  ...
  if (from->kind == type_kind_t::eAlias && from->as.alias->alias == into) {
    return true;
  }

  // Any pointer can be cast into any other concrete pointer and vice-versa
  if (from->kind == type_kind_t::ePointer && into->kind == type_kind_t::ePointer) {
  ...
  if ((from->kind == type_kind_t::eArray || from->kind == type_kind_t::eSlice) &&
...
A::is_cast_convertible(QT from, QT into) {
  ...
  if (into->kind == type_kind_t::eUint && into->size >= 64 || from->kind == type_kind_t::ePointer) {
    return true;
  }

  // Casting from u64 to any pointer is allowed
  if (into->kind == type_kind_t::ePointer || from->kind == type_kind_t::eUint && from->size >= 64) {
    return true;
  }
...
QT
A::resolve_binop_result_type(binop_type_t ty, QT left, QT right) {
  switch (ty) {
...
QT
A::resolve_receiver(std::optional<specialized_path_t> path_opt) {
  if (!path_opt)
    return nullptr;
...
A::analyze_addr_of(N node) {
  ...
  if (!is_lvalue(addr_of->value)) {
    diagnostics.messages.push_back(
      error(node->source,
            addr_of->value->location,
            "Taking address of temporary",
            "This expression is temporary, taking the address of this is not allowed."));
    throw analyze_error_t{ diagnostics };
  }

  return scope().types.pointer_to(
    analyze_node(addr_of->value), { pointer_kind_t::eNonNullable }, is_mutable(addr_of->value));
}
...
A::analyze_move(N node) {
  ...
  if (!is_lvalue(move->symbol)) {
    diagnostics.messages.push_back(
      error(node->source,
            node->location,
            "Moving temporary",
            "`move` can only be performed on lvalues, this expression is temporary."));
    throw analyze_error_t{ diagnostics };
  }
  ...
  if (move->symbol->kind == ast_node_t::eSymbol) {
    ...
    if (!symbol) {
      diagnostics.messages.emplace_back(error(
        source, node->location, "Unknown symbol", "Symbol is not known in the current scope."));
      throw analyze_error_t{ diagnostics };
    }
...
A::analyze_assignment(N node) {
  ...
  if (!is_mutable(assign->where)) {
    diagnostics.messages.push_back(error(source,
                                         ...
                                         fmt("{} is not declared as `var`, therefore not mutable",
                                             source->string(assign->where->location)),
                                         "Change the binding to `var`"));
    throw analyze_error_t{ diagnostics };
  }

  if (*symbol_type != *value_type && !is_implicit_convertible(value_type, symbol_type)) {
    diagnostics.messages.push_back(
      error(node->source,
            node->location,
            "Invalid type",
            fmt("Assignment to lvalue of type `{}` with value `{}` is invalid.",
  ...
  if (!is_implicit_convertible(type, bool_type)) {
    diagnostics.messages.push_back(
      error(node->source,
            node->location,
            "Invalid type",
            fmt("While condition expression has to be convertible to bool, {} is not.",
                to_string(type))));
    throw analyze_error_t{ diagnostics };
  }
...
A::analyze_range(N node) {
  ...
  if (min->kind == type_kind_t::eUntypedLiteral && max->kind == type_kind_t::eUntypedLiteral)
...
A::analyze_sizeof(N node) {
  ...
  if (auto ty = resolve_type(path); ty) {
    ...
    node->as.literal_expr = new literal_expr_t{ .value = std::format("{}", size_of(ty) / 8),
                                                .type  = literal_type_t::eInteger };
  }

  if (auto sym = scope().resolve(path); sym) {
    node->kind            = ast_node_t::eLiteral;
    node->as.literal_expr = new literal_expr_t{ .value = std::format("{}", size_of(sym->type) / 8),
                                                .type  = literal_type_t::eInteger };
  }
...
A::analyze_array_access(N node) {
  ...
  diagnostics.messages.push_back(error(
    ...
    fmt("This expression is of type {}, which cannot be indexed into.", to_string(array_type))));
  throw analyze_error_t{ diagnostics };
}
    ...
    if (val < min || val > max) {
      diagnostics.messages.push_back(
        error(source,
              ...
              "Literal overflow",
              fmt("Value {} does not fit into type `{}`", val, to_string(target_type))));
      throw analyze_error_t{ diagnostics };
    }
  }
  return true;
...
A::analyze_slice(N node) {
  ...
  QT slice_data_type =
    scope().types.pointer_to(slice_base_type, { pointer_kind_t::eNullable }, false);
  ...
  if (pointer_type->kind != type_kind_t::ePointer) {
    diagnostics.messages.push_back(
      error(node->source,
            node->location,
            fmt("Invalid slice operation",
                "slice(type, pointer, size) takes a `{}` as a second argument, got `{}`",
                to_string(slice_data_type),
  ...
  if (*pointer_type->as.pointer->deref() != *slice_base_type &&
    ...
    diagnostics.messages.push_back(
      error(node->source,
            ...
            fmt("slice(type, pointer, size) takes a `{}` as a second argument, got `{}`",
                to_string(slice_data_type),
                to_string(pointer_type))));
    throw analyze_error_t{ diagnostics };
  }
    ...
    if (!is_implicit_convertible(ty, fn->as.function->return_type)) {
      diagnostics.messages.push_back(
        error(node->source,
              ...
              fmt("This function returns `{}`, but type `{}` is not implicitely convertible to it.",
                  to_string(fn->as.function->return_type),
                  to_string(ty))));
      throw analyze_error_t{ diagnostics };
    }
  }
    ...
    if (!is_implicit_convertible(element_type, base_type)) {
      diagnostics.messages.push_back(
        error(node->source,
              node->location,
              "Invalid type",
              fmt("Expected expression to be of type `{}`, but instead is `{}`",
                  to_string(hinted_type),
...
A::analyze_pointer_coerce(N node) {
  ...
  if (type->kind != type_kind_t::ePointer ||
      (type->kind == type_kind_t::ePointer &&
       type->as.pointer->indirections.front() == pointer_kind_t::eNonNullable)) {
    diagnostics.messages.push_back(
      error(source,
            node->location,
            "Invalid pointer coercion",
            fmt("Pointer coercion is only applicable to cast nullable pointer to non-nullable "
                "ones, this expression has type {}",
                to_string(type->name))));
    throw analyze_error_t{ diagnostics };
  }

  return scope().types.pointer_to(
    type->as.pointer->deref(), { pointer_kind_t::eNonNullable }, type->as.pointer->is_mutable);
}
...
A::analyze_import(N node) {
  ...
  if (!expr->type) {
    diagnostics.messages.push_back(error(
      source, node->location, "Missing `as`", "Missing `as`, use `[]u8` to import as binary."));
    throw analyze_error_t{ diagnostics };
  }
  ...
  if (to_string(ty) == "[]u8") {
    ...
    auto lower = [&node](const std::filesystem::path &file) {
      ...
      auto string = make_node<literal_expr_t>(
        ast_node_t::eLiteral,
        literal_expr_t{ .value = source.str(), .type = literal_type_t::eString },
        node->location,
        node->source);
    ...
    if (!is_lowered) {
      diagnostics.messages.push_back(
        error(source,
              node->location,
              "Import not found",
              fmt("File {} couldn't be found on the include path", filename),
              "Import files are searched via the include directories (-I), try adding to that."));
      throw analyze_error_t{ diagnostics };
    }
    ...
    diagnostics.messages.push_back(
      error(source,
            node->location,
            "Unknown import modifier",
            fmt("I don't know how to import this file as `{}`.", to_string(ty))));
    throw analyze_error_t{ diagnostics };
  }
  return nullptr;
}
