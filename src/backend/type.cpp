...
bool
type_t::is_numeric() const {
  return kind == eInt || kind == eUint || kind == eFloat;
}

bool
type_t::is_signed() const {
  return kind == eInt;
}

bool
type_t::operator==(const type_t &other) const {
...
size_t
size_of(SP<type_t> type) {
  return size_of(*type);
}

size_t
alignment_of(SP<type_t> type) {
  return alignment_of(*type);
}

size_t
size_of(const type_t &type) {
...
size_t
alignment_of(const type_t &type) {
  if (type.kind != type_kind_t::ePointer)
...
std::string
to_string(SP<type_t> type) {
  return to_string(*type);
}

std::string
to_string(const type_t &type) {
  std::stringstream ss;
...
pointer_t::deref() const {
  if (indirections.size() == 1) {
    ...
    type->as.pointer = new pointer_t{ .is_mutable   = this->is_mutable,
...
pointer_t::pointer_to(pointer_kind_t kind, SP<type_t> base, bool is_mutable) {
  ...
  if (base->kind == type_kind_t::ePointer) {
    ...
    ptr->indirections.insert(
      ptr->indirections.begin(), base_ptr->indirections.begin(), base_ptr->indirections.end());
  }
