...
SP<symbol_t> scope_t::resolve(const specialized_path_t &identifier) {
...
void scope_t::remove(const std::string &identifier) {
  if (symbols.contains(identifier)) {
...
SP<symbol_t> scope_t::add(const specialized_path_t &identifier, SP<type_t> type, bool is_mutable) {
  return add(to_string(identifier), type, is_mutable);
}

void scope_t::add_template(SP<template_decl_t> node) {
...
scope_t::candidates(const specialized_path_t &path) {
  ...
  for (auto &template_ : templates) {
    ...
    for (auto i = 0; i < path.segments.size(); i++) {
      ...
      if (path.segments[i].types.size() != template_->name.segments[i].bindings.size()) match = false;
      if (!match) break;
    }
