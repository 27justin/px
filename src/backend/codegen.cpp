...
#define VISITOR(name) SP<llvm_value_t> codegen_t::visit_##name(SP<ast_node_t> node)
...
void
codegen_t::compile_to_llvm_ir(std::optional<std::string> filename) {
  std::error_code EC;
  ...
  if (filename == "-") {
    ...
    llvm::raw_fd_ostream ir_dest(
      filename.value_or(
        std::filesystem::path(source->name()).replace_extension(".ll").filename().string()),
      EC,
      llvm::sys::fs::OF_None);
...
void
codegen_t::set_opt_level(const std::string &level) {
...
codegen_t::codegen_t(std::shared_ptr<source_t> src, semantic_info_t &&s)
  : source(src)
  , info(std::move(s)) {
...
void
codegen_t::link_external_symbol(const std::string &name, SP<type_t> type) {
  ...
  switch (type->kind) {
    ...
    default: {
      llvm::GlobalVariable *variable = new llvm::GlobalVariable(
        *module, llvm_type, false, llvm::GlobalValue::ExternalLinkage, nullptr, name);
  ...
  for (auto &[external, type] : info.imported_symbols) {
    if (type->kind != type_kind_t::eEnum) {
      ...
      for (auto &[name, val] : members) {
        ...
        scope()->set(
          to_string(path),
          std::make_shared<llvm_value_t>(llvm::ConstantInt::get(intty, val), intty, true));
      }
    }
  }
...
void
codegen_t::contract_emit_dynamic_dispatcher(SP<type_t>         contract,
  ...
  llvm::FunctionType *dispatcher_fn =
    llvm::FunctionType::get(receiver_fn->getReturnType(), params, receiver_fn->isVarArg());
  ...
  auto func = llvm::Function::Create(
    dispatcher_fn, llvm::GlobalValue::LinkOnceODRLinkage, to_string(dispatcher_path), *module);
  ...
  auto offset = std::distance(contract->as.contract->requirements.begin(),
                              contract->as.contract->requirements.find(symbol_name));

  // Load the `offset`-th pointer from the vtable
  auto func_ptr = builder->CreateStructGEP(
    llvm::StructType::getTypeByName(*context, "contract." + to_string(contract->name)),
    vtable,
    offset,
    "method_ptr_addr");
  ...
  auto ret_val =
    builder->CreateCall(dispatcher_fn,
                        dyn_func,
                        args,
                        dispatcher_fn->getReturnType()->isVoidTy() ? "" : "vtable_dispatch");
  ...
  scope()->set(to_string(dispatcher_path),
               std::make_shared<llvm_value_t>(func, dispatcher_fn, true));
  builder->SetInsertPoint(current_block);
}

llvm::Type *
codegen_t::ensure_type(SP<type_t> type) {
  ...
  switch (type->kind) {
    ...
    case type_kind_t::eContract: {
      ...
      if (result = llvm::StructType::getTypeByName(*context, "contract"); !result) {
        std::vector<llvm::Type *> types{ llvm::PointerType::get(*context, 0),
    ...
    case type_kind_t::eFloat: {
      result =
        type->size == 32 ? llvm::Type::getFloatTy(*context) : llvm::Type::getDoubleTy(*context);
      break;
    }
...
codegen_t::cast(SP<type_t> type, const llvm_value_t &value) {
  ...
  if (target_type->isIntegerTy() && value.type->isIntegerTy()) {
    // Int cast
    return llvm_value_t(
      builder->CreateIntCast(load(value).value, target_type, type->is_signed()), target_type, true);
  }
  ...
  if ((target_type->isPointerTy() && value.type->isIntegerTy()) ||
    ...
    return llvm_value_t(
      builder->CreateBitOrPointerCast(load(value).value, target_type), target_type, true, type);
  }

  if (type->kind == type_kind_t::eContract) {
    // Create the contract object
    auto generic_contract_type = llvm::StructType::getTypeByName(*context, "contract");
    auto constrained_contract_type =
      llvm::StructType::getTypeByName(*context, "contract." + to_string(value.base_type->name));
    ...
    auto field_vtable_ptr =
      builder->CreateStructGEP(generic_contract_type, contract_ptr, 0, "vtable_field");
    auto field_data_ptr =
      builder->CreateStructGEP(generic_contract_type, contract_ptr, 1, "data_field");
...
codegen_t::field_type(SP<type_t> ty, const std::string &name) {
  switch (ty->kind) {
    ...
    case type_kind_t::eSlice: {
      return name == "data" ? llvm_type_cache.at(llvm::PointerType::get(*context, 0))
                            : llvm_type_cache.at(builder->getIntNTy(64));
    }
    default:
      throw std::runtime_error("Unknown type in field_index");
  }
  ...
  switch (expr->value->kind) {
    case ast_node_t::eSymbol: {
      ...
      return std::make_shared<llvm_value_t>(
        ...
        llvm::PointerType::get(*context, 0),
        true,
        pointer_t::pointer_to(pointer_kind_t::eNonNullable, left->base_type));
    }
...
codegen_t::slice_create_from_array(const llvm_value_t &stack_array) {
  ...
  slice =
    builder->CreateInsertValue(slice,
                               llvm::ConstantInt::get(builder->getInt64Ty(), arr->getNumElements()),
                               { 1 },
                               "slice.size");
...
llvm_value_t
codegen_t::slice_create_from_parts(SP<type_t>          base_type,
                                   const llvm_value_t &pointer,
                                   const llvm_value_t &size) {
  // TODO: is_mutable: false?
  ...
  slice = builder->CreateInsertValue(
    slice, cast(llvm_type_cache.at(builder->getIntNTy(64)), load(size)).value, { 1 }, "slice.size");
...
VISITOR(function_decl) {
  ...
  return scope()->set(to_string(*current_binding),
                      std::make_shared<llvm_value_t>(func, llvm_fn, false));
}
...
VISITOR(function_impl) {
  ...
  auto func    = llvm::Function::Create(llvm_fn,
                                     current_linkage.value_or(llvm::GlobalValue::ExternalLinkage),
                                     to_string(*current_binding),
                                     *module);
  auto ret_val = scope()->root().set(to_string(*current_binding),
                                     std::make_shared<llvm_value_t>(func, llvm_fn, false));
  ...
  for (uint64_t i = 0; i < impl->declaration.parameters.size(); ++i) {
    ...
    scope()->set(param.name,
                 std::make_shared<llvm_value_t>(llvm_value, ty, is_rvalue, param.resolved_type));
  }
...
VISITOR(slice_expr) {
  ...
  return std::make_shared<llvm_value_t>(slice_create_from_parts(
    slice->resolved_type, *visit_node(slice->pointer), *visit_node(slice->size)));
}
...
VISITOR(block) {
  ...
  if (!block->resolved_return_type ||
      (block->resolved_return_type &&
       ensure_type(block->resolved_return_type)->isVoidTy() == true)) {
  ...
  if (block->has_implicit_return) {
    ...
    return_value =
      std::make_shared<llvm_value_t>(builder->CreateAlloca(return_type, 0, nullptr, "blockval"),
                                     return_type,
                                     false,
                                     block->resolved_return_type);
  ...
  for (auto it = scope->defer_stack.rbegin(); it != scope->defer_stack.rend(); ++it) {
    visit_node(*it);
  }
  ...
  if (block->has_implicit_return) {
    return std::make_shared<llvm_value_t>(load(return_value).value,
                                          ensure_type(block->resolved_return_type),
                                          true,
                                          block->resolved_return_type);
  } else {
    return nullptr;
...
llvm::Value *
codegen_t::string_literal_global_create(const std::string &str) {
...
codegen_t::resolve_member(const llvm_value_t &val, const std::string &member) {
  ...
  bool is_lvalue = (current_type->kind == type_kind_t::ePointer || val.is_rvalue == false);
  ...
  return std::make_shared<llvm_value_t>(
    member_data, ensure_type(field_type), result_is_rvalue, field_type);
}
...
VISITOR(symbol) {
  ...
  if (left) {
    ...
    ufcs.segments.insert(ufcs.segments.begin(),
                         symbol->path.segments.begin(),
                         symbol->path.segments.end() - ufcs.segments.size());
...
bool
codegen_t::is_scalar_binop(binop_type_t ty) {
    ...
    if (is_scalar_binop(expr->op)) {
      ...
      switch (expr->op) {
        ...
        case binop_type_t::eBitAnd:
          return std::make_shared<llvm_value_t>(
            builder->CreateAnd(load(left).value, load(right).value), left_type, true);

        case binop_type_t::eBitOr:
          return std::make_shared<llvm_value_t>(
            builder->CreateOr(load(left).value, load(right).value), left_type, true);

        case binop_type_t::eXor:
          return std::make_shared<llvm_value_t>(
            builder->CreateXor(load(left).value, load(right).value), left_type, true);

        case binop_type_t::eBitShiftLeft:
          return std::make_shared<llvm_value_t>(
            builder->CreateShl(load(left).value, load(right).value), left_type, true);

        case binop_type_t::eBitShiftRight:
          // Logical shift (lshr) for unsigned, Arithmetic shift (ashr) for signed
          if (use_signed_cmp) {
            return std::make_shared<llvm_value_t>(
              builder->CreateAShr(load(left).value, load(right).value), left_type, true);
          } else {
            return std::make_shared<llvm_value_t>(
              builder->CreateLShr(load(left).value, load(right).value), left_type, true);
          }

        default:
          throw std::runtime_error(
            "Internal Compiler Error: Unhandled non-scalar binary operation.");
          break;
      }
...
VISITOR(array_access) {
  ...
  return std::make_shared<llvm_value_t>(
    address, element_type, false, expr->value->type->base_type());
}
...
VISITOR(deref) {
  ...
  return std::make_shared<llvm_value_t>(load(value).value,
...
VISITOR(for) {
  ...
  if (stmt->condition && stmt->condition->kind == ast_node_t::eRangeExpr) {
    builder->CreateStore(
      builder->CreateBinOp(llvm::Instruction::Add,
                           load(init->type, *init).value,
...
VISITOR(enum) {
  ...
  for (auto &[name, val] : enum_layout->values) {
    ...
    scope()->set(to_string(binding),
                 std::make_shared<llvm_value_t>(
                   llvm::ConstantInt::get(storage_type, val), storage_type, true));
  }
  return nullptr;
}
...
VISITOR(array_initializer) {
  ...
  for (size_t i = 0; i < expr->values.size(); ++i) {
    ...
    llvm::Value *element_ptr = builder->CreateInBoundsGEP(
      array_type, array_ptr, { builder->getInt32(0), builder->getInt32((uint32_t)i) });

    builder->CreateStore(loaded_value.value, element_ptr);
  }

  return std::make_shared<llvm_value_t>(array_ptr,
                                        array_type,
                                        true, // We don't need to load any further, loading this
                                              // would yield the first element.
                                        node->type);
}
...
VISITOR(unary) {
  ...
  if (expr->op == token_type_t::operatorMinus) {
    if (value->type->isIntegerTy())
      ...
      return std::make_shared<llvm_value_t>(
        builder->CreateFNeg(load(value).value, "float_neg"), value->type, true);
  }

  // ! (negate)
  if (expr->op == token_type_t::operatorExclamation) {
    return std::make_shared<llvm_value_t>(
      builder->CreateNot(load(value).value, "bool_not"), value->type, true);
  }
...
VISITOR(assignment) {
  ...
  if (expr->where->kind == ast_node_t::eDeref) {
    ...
    left         = std::make_shared<llvm_value_t>(
      address.value, address.type, address.is_rvalue, address.base_type);
  } else {
...
llvm::Value *
codegen_t::contract_emit_static_vtable(SP<type_t> contract_ty, SP<type_t> implementor) {
  auto static_variable_name =
    to_string(contract_ty->name) + "." + to_string(implementor->name) + ".vtable";
